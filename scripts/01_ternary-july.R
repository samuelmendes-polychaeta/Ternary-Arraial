...
####JULY####
prop <- sweep(abund_july, 1, rowSums(abund_july), "/")

groups <- env_july$regiao
...
D_KG_bar_Exposed <- sapply(1:11, function(i) mean(D_KG_Exposed[i, -i]))
S_BC_bar_Exposed<- sapply(1:11, function(i) mean(S_BC_Exposed[i, -i]))
R_beta_bar_Exposed <- sapply(1:11, function(i) mean(R_beta_Exposed[i, -i]))
...
D_KG_bar_Protected <- sapply(1:12, function(i) mean(D_KG_Protected[i, -i]))
S_BC_bar_Protected<- sapply(1:12, function(i) mean(S_BC_Protected[i, -i]))
R_beta_bar_Protected<- sapply(1:12, function(i) mean(R_beta_Protected[i, -i]))
...
final_plot <- ptri_july + p_bar + 
  plot_layout(guides = 'collect') &  # This collects and shares the legend
  theme(legend.position = 'bottom')   # Position the shared legend  
