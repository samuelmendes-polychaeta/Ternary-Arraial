...
summary_df <- plot_df %>%
  ...
  summarise(
    median = median(RLQ1, na.rm = TRUE),
    q25 = quantile(RLQ1, 0.25, na.rm = TRUE),
    q75 = quantile(RLQ1, 0.75, na.rm = TRUE),
    .groups = "drop"
  )


ggplot(plot_df, aes(x = RLQ1, y = modality)) +
  ...
  theme_bw() +
  labs(
    x = "Species position on RLQ axis 1",
    y = "Trait modalities"
  )
